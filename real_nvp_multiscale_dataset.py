...
from utils import (
    read_pictures_from_directory_as_numpy,
    convert_numpy_images_to_tensors_with_rescaling,
    load_latent_space_samples,
    save_numpy_array_to_directory)
...
tf.flags.DEFINE_string("data_path", None, "Path to the data.")
...
tf.flags.DEFINE_string("encoding_path", "", "Path to latent space samples")
...
tf.flags.DEFINE_string("sampling_size", "normal", "Resolution of the output image")
...
class RealNVP(object):
    ...
    def __init__(self, hps, sampling=False):
        ...
        if FLAGS.dataset == "celeba":
            ...
            image_size = 64
            images = read_pictures_from_directory_as_numpy(FLAGS.data_path)
            x_in = convert_numpy_images_to_tensors_with_rescaling(images, image_size=image_size, worker_device=device)
            #print("_________________GETTING CUSTOM DATA____________")
            #self.x_orig = x_orig = x_in
        else:
        ...
        print("Hello there: {0}".format(FLAGS.sampling_size))
        if sampling and FLAGS.sampling_size=="normal":
            # SAMPLES
            if FLAGS.encoding_path == "":
                sample = standard_normal_sample([100] + final_shape)
                sample_size = 100
            else:
                sample = load_latent_space_samples(FLAGS.encoding_path)
...
#                 train=True)
            ...
            sample = tf.reshape(sample, [sample_size, image_size, image_size, 3])
            sample = tf.transpose(
            ...
            sample = tf.transpose(
                ...
                [0, 2, 1, 3])
            self.sample = tf.cast(sample, tf.uint8)
            #tf.summary.image(
            #    "samples",
        ...
        elif sampling and FLAGS.sampling_size=="big":
            # SAMPLES x2
            final_shape[0] *= 2
            final_shape[1] *= 2
            ...
            self.sample = tf.cast(big_sample, tf.uint8)
            #tf.summary.image(
            #    "big_sample",
...
def train_model(hps, logdir):
    ...
    with tf.Graph().as_default():
        with tf.device(tf.train.replica_device_setter(0)):
            ...
            #print("ckpt_state: {0} | ckpt_state.model_checkpoint_path: {1}".format(ckpt_state, ckpt_state.model_checkpoint_path))
            ...
            while True:
                ...
                if local_step != 0 and local_step % 1000 == 0 or (local_step + 1) == FLAGS.train_steps:
...
def decoding(hps, logdir):
    with tf.Graph().as_default():
        with tf.device("/gpu:0"):
            with tf.variable_scope("model"):
                model = RealNVP(hps, sampling=True)
            ...
            fetches = [model.sample]
            samples = sess.run(fetches)[0][0]
            save_numpy_array_to_directory(samples, 'sample.jpg', directory='./decoding')

            

def encoding(hps, logdir):
    with tf.Graph().as_default():
        with tf.device("/gpu:0"):
            with tf.variable_scope("model"):
                model = RealNVP(hps)
            ...
            if ckpt_state and ckpt_state.model_checkpoint_path:
                ...
                saver.restore(sess, ckpt_state.model_checkpoint_path)
            tf.train.start_queue_runners(sess)
            #all_embedding = numpy.array([], dtype=numpy.float32)
            
            if FLAGS.mode == "encoding":
                fetches = [model.z_out]
                embedding = sess.run(fetches)[0]
            elif FLAGS.mode == "encoding_batch":
                for i in range(0,386):
                    print("{}/386".format(i+1))
                    coord = tf.train.Coordinator()
                    ...
                    if i == 0: 
                        ...
                        all_embedding = numpy.append(embedding, all_embedding, axis = 0)
                numpy.save(FLAGS.encoding_path, all_embedding)
            
            #all_embedding = numpy.append(all_embedding, embedding, axis=0)
            with open(FLAGS.encoding_path, 'w') as f:
...
def main(unused_argv):
    ...
    elif FLAGS.mode == "encoding" or FLAGS.mode == "encoding_batch":
        encoding(hps=hps, logdir=FLAGS.logdir)
    elif FLAGS.mode == "decoding":
        print(FLAGS.encoding_path)
        decoding(hps=hps, logdir=FLAGS.logdir)
    else:
        hps.batch_size = 100
